...
const Card = ({ project, index, total }: { project: any, index: number, total: number }) => {
  ...
  const { scrollYProgress } = useScroll({
    target: cardRef,
    offset: ["start start", "end start"] // from when top hits viewport top, to when bottom hits viewport top
  });

  // Scale down smoothly as card scrolls up and gets "covered"
  ...
  return (
    <div ref={cardRef} className="h-auto">
      <motion.article 
